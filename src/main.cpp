...
#if defined(NDEBUG)
# error "zzcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x861bf03c0a481d2deea18f728bd2f88cbb063bda2ea732d1a46e57aa29c842b3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // zzcoin: starting difficulty is 1 / 2^12
...
{
    // zzcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // zzcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 60; // zzcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // zzcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // zzcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // zzcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x861bf03c0a481d2deea18f728bd2f88cbb063bda2ea732d1a46e57aa29c842b3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is ZZCoin. 2018-07-23";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1532320519;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1098445;
        ...
        assert(block.hashMerkleRoot == uint256("0xda6048aff864ed70cd265c0a2a26ec924024fc4b2442e07a390c442830739c5a"));

        // If genesis block hash does not match, then generate new genesis hash.
        if (true && block.GetHash() != hashGenesisBlock) //<==완료후에는 false
        {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // zzcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("zzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("zzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static zzcoinMiner(CWallet *pwallet)
{
    printf("zzcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zzcoin-miner");
    ...
    try { loop {
        ...
        printf("Running zzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&zzcoinMiner, pwallet));
}
